...
### Inteligencia de logs Kubernetes

Cuando un workload o Pod Kubernetes tiene un stream de logs abierto, el panel de topología **Intelligent** puede medir las líneas que la pestaña de terminal conserva en ese momento. Informa cantidad de líneas, tasa de error, advertencias, firmas de errores recurrentes y conteos de palabras habituales de fallo como `error`, `failed`, `timeout`, `panic`, `OOMKilled`, `refused` y `unavailable`. Las firmas normalizan timestamps, UUIDs y valores numéricos para revisar fallos repetidos como un solo patrón.

La acción **Revisar relaciones** analiza el mismo stream en busca de referencias a servicios Kubernetes internos, por ejemplo `service.namespace.svc.cluster.local`. Si el servicio o workload referenciado existe en la topología de la aplicación, KUA crea una sugerencia `calls` revisable. Nunca confirma una dependencia automáticamente: sigue siendo obligatorio usar el flujo existente de aceptar/rechazar.

Esta primera versión está acotada a la sesión: analiza la ventana en memoria de la terminal (hasta su límite de retención), no ejecuta otra consulta de logs a Kubernetes y no alimenta el scheduler de 30 minutos. Al cerrar la pestaña de logs se elimina la fuente de datos. Los contadores son heurísticas deterministas, no un score de anomalía basado en ML, y cuentan líneas coincidentes, no eventos con severidad estandarizada del proveedor. Las tasas históricas y las alertas requieren una futura canalización de agregación persistida.

Antes de mostrar evidencia de relaciones, KUA redacta credenciales comunes, tokens bearer, credenciales en URLs, query strings de URLs y valores con forma de correo. Esto reduce la exposición, pero no garantiza anonimización; los operadores deben tratar los logs y las firmas de error mostradas como datos operativos sensibles.
