...
- KUA now has an optional Account header button (hidden unless `VITE_CONTROL_PLANE_URL`
  is configured) that connects the frontend to the KUA Control Plane: sign in/out with
  Google, and a read-only account panel showing the current plan (Free/Pro/Team) and
  which cloud features it unlocks. Every local/free feature keeps working exactly as
  before with no control plane configured — the button simply doesn't render. Clicking
  "Sign in" against a control plane that hasn't configured Google OAuth yet shows a clear
  "not available yet" message instead of navigating to a raw JSON error page.
- Fixed the KUA Control Plane silently ignoring its own `.env` file: `npm start` never
  actually loaded it (no `dotenv`), so every value in `.env` fell back to defaults
  regardless of what was configured, contradicting the README's own setup instructions.
...
- Console sessions now have contextual "Open in Console" one-click actions wherever a
  ...
  SSH/RDP, a GCP Cloud Run service's Logs tab gets an "Open in Console" action, and a
  Vercel deployment row gets one too. Every session opened from Architecture,
  Observability or these new actions now correctly threads its real environment and
  application id into the session (previously every session's audit entry read
  `environment: default` regardless of where it was opened from). GCP Cloud Run's
  Console launcher no longer requires typing a project id — it was always resolved
  from the credential profile server-side, but a stricter client-side check blocked
  that fallback from ever being used. This closes the Console epic (#36-43). See
  [console sessions](./architecture/console-sessions.md).
- GCP and Vercel join the Console with honest capabilities: GCP Cloud Run logs and
  Vercel deployment logs can now be tailed as sessions (no shell — both are one-way log
  streams), while GCP Cloud Shell is explicitly marked unavailable with the reason shown
  in the launcher, since there's no way to start it from a stored service-account
  credential. See [console sessions](./architecture/console-sessions.md).
...
- The Console is now reachable from a global header button in every module, in
  ...
  target, status) with close/reconnect actions, plus a launcher to start new
  Local Shell or Kubernetes logs/exec sessions. Both surfaces read the same
  session store, so nothing opened in one is ever invisible in the other.
  EC2 SSH/RDP remain reachable only from the AWS view for now.
