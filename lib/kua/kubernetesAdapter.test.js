...
function fixture({ eventsError = false, nodesError = false, contextName: fixtureContext = 'docker-desktop' } = {}) {
  const configBuilder = contextName => ({
    kubeConfig: {
      getContexts: () => [{ name: fixtureContext, cluster: 'local' }],
      ...
      makeApiClient(Api) {
        ...
        if (Api === CoreV1Api) return {
          ...
          listNode: nodesError
            ...
            : response([{
              metadata: { uid: 'node-uid', name: 'node-a', labels: { 'node.kubernetes.io/instance-type': 'm5.large' } },
              spec: { providerID: 'aws:///us-east-1a/i-002905f9976a29892' },
        ...
        if (Api === EventsV1Api) return {
          listEventForAllNamespaces: eventsError
            ...
            : response([{
              type: 'Warning', reason: 'BackOff', note: 'restart loop', metadata: { namespace: 'orders', name: 'api.1' },
              regarding: { kind: 'Deployment', namespace: 'orders', name: 'api' },
            }]),
        };
...
test('discovers Kubernetes topology with UID identity, declared evidence and health', async () => {
  ...
  const [warning] = result.health[0].warningEvents;
  assert.equal(warning.regardingKind, 'Deployment');
  assert.equal(warning.regardingName, 'api');
  assert.equal(warning.regardingNamespace, 'orders');
...
test('resolves a cluster Node to the cloud instance it actually runs on', async () => {
  const result = await fixture().preview({ provider: 'generic', contexts: ['docker-desktop'] });
  const node = result.nodes.find(item => item.resourceType === 'node');

  assert.equal(node.instanceId, 'i-002905f9976a29892');
  assert.equal(node.availabilityZone, 'us-east-1a');
  assert.equal(node.instanceType, 'm5.large');
  // A node group's instances are created by the Auto Scaling group, so CloudFormation discovery
  // never sees them: this ARN is what lets the running instance correlate with AWS at all.
  assert.equal(node.arn, undefined, 'no account is known from a plain context name, so no ARN is invented');
  assert.ok(node.evidence.some(entry => entry.type === 'cloud_instance'));
});

test('derives the instance ARN from an EKS context, which names the account explicitly', async () => {
  const context = 'arn:aws:eks:us-east-1:123456789012:cluster/orders';
  const result = await fixture({ contextName: context }).preview({ provider: 'aws', contexts: [context] });
  const node = result.nodes.find(item => item.resourceType === 'node');

  assert.equal(node.arn, 'arn:aws:ec2:us-east-1:123456789012:instance/i-002905f9976a29892');
  assert.equal(node.accountId, '123456789012');
