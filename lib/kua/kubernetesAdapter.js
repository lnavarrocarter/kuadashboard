...
function nodeFromItem(context, item, resourceType, health = {}) {
  ...
  if (!uid) return null;
  const kind = item.kind || resourceType;
  return {
  ...
  async preview({ provider = 'generic', contexts: requestedContexts, namespaces } = {}) {
    ...
    for (const context of contexts) {
      ...
      try {
        ...
        const workloadItems = [...deployments, ...statefulSets, ...daemonSets].filter(item => namespaceAllowed(item, namespaces));
        ...
        const nodes = [
          ...workloadItems.map(item => nodeFromItem(contextName, item, String(item.kind || '').toLowerCase(), workloadHealth(item))),
