...
const KIND_BY_RESOURCE_TYPE = {
  ...
  pvc: 'PersistentVolumeClaim', ingress: 'Ingress', node: 'Node',
};
...
// A cluster Node IS the running cloud instance: the EC2 instances of an EKS node group are created
// by the Auto Scaling group at runtime, so CloudFormation discovery never sees them. `spec.providerID`
// is the authoritative link, and the account only comes from an explicit EKS context ARN — without
// one an ARN would be guesswork, so the instance identity is still recorded but left uncorrelated.
const EKS_CONTEXT_ARN = /^arn:aws:eks:([^:]+):(\d+):cluster\//;
const AWS_PROVIDER_ID = /^aws:\/\/\/([a-z0-9-]+)\/(i-[0-9a-f]+)$/i;

function awsInstanceIdentity(item, context) {
  const match = AWS_PROVIDER_ID.exec(String(item.spec?.providerID || ''));
  if (!match) return null;
  const [, availabilityZone, instanceId] = match;
  const contextMatch = EKS_CONTEXT_ARN.exec(String(context));
  const region = contextMatch?.[1] || availabilityZone.replace(/[a-z]$/i, '');
  const accountId = contextMatch?.[2] || '';
  return {
    instanceId,
    availabilityZone,
    region,
    ...
    arn: accountId ? `arn:aws:ec2:${region}:${accountId}:instance/${instanceId}` : null,
  };
}
...
function nodeFromItem(context, item, resourceType, health = {}) {
  ...
  const instance = resourceType === 'node' ? awsInstanceIdentity(item, context) : null;
  return {
    ...
    evidence: [
      { type: 'kubernetes_uid', sourceId: sourceId(context), values: [uid] },
      ...(instance ? [{ type: 'cloud_instance', sourceId: sourceId(context), values: [instance.instanceId] }] : []),
    ],
  };
}
...
const RELATIONSHIP_CONFIDENCE = {
  ingress_backend: 1,
  // The scheduler already decided this: spec.nodeName is a fact, not an inference.
  pod_node_assignment: 1,
  env_service_dns: 0.9,
  env_service_name: 0.75,
};
...
function relationship(context, source, target, relationType, evidence) {
  return {
    ...
    relationType,
    status: 'suggested',
    confidence: RELATIONSHIP_CONFIDENCE[evidence.type] ?? 0.95,
...
function clusterNodeHealth(item) {
...
// Only env var keys shaped like "reaches another service" are trusted for a bare (non fully-qualified)
// hostname, since a bare word alone (e.g. an env value of "true") is not distinctive evidence on its own.
const SERVICE_ENV_KEY_HINT = /host|url|uri|endpoint|service|addr/i;
const K8S_SERVICE_DNS_RE = /^([a-z0-9]([a-z0-9-]*[a-z0-9])?)\.([a-z0-9]([a-z0-9-]*[a-z0-9])?)\.svc(?:\.cluster\.local)?$/i;
const BARE_HOST_RE = /^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/i;
...
function hostFromEnvValue(value) {
  ...
  const hostPortMatch = trimmed.match(/^([a-z0-9.-]+):\d+$/i);
  return hostPortMatch ? hostPortMatch[1] : trimmed;
}

// Resolves an env var value to a same-cluster Service/workload reference without downloading or
// running anything — the same "read config metadata, never execute" approach used for AWS discovery.
function workloadServiceEnvReferences(item) {
  const references = [];
  for (const container of item.spec?.template?.spec?.containers || []) {
    for (const env of container.env || []) {
      if (typeof env.value !== 'string' || !env.value) continue;
      const host = hostFromEnvValue(env.value);
      if (!host) continue;
      const dnsMatch = host.match(K8S_SERVICE_DNS_RE);
      if (dnsMatch) {
        references.push({ service: dnsMatch[1], namespace: dnsMatch[3], key: env.name, evidence: 'env_service_dns' });
        continue;
      }
      if (BARE_HOST_RE.test(host) && SERVICE_ENV_KEY_HINT.test(env.name || '')) {
        references.push({ service: host, namespace: null, key: env.name, evidence: 'env_service_name' });
      }
    }
  }
  return [...new Map(references.map(reference => [`${reference.service}:${reference.namespace || ''}:${reference.key}`, reference])).values()];
}
...
class KubernetesAdapter {
  ...
  async preview({ provider = 'generic', contexts: requestedContexts, namespaces } = {}) {
    ...
    for (const context of contexts) {
      ...
      const capabilities = { discovery: true, stableIdentity: true, relationshipEvidence: true, healthSignals: true, events: true, clusterNodes: true };
      try {
        ...
        // Cluster-scoped reads a namespace-scoped role is often not allowed to make: these degrade
        // their capability instead of failing the whole discovery.
        const optionalRequests = [
          core.listNode ? core.listNode() : Promise.reject(new Error('listNode is unavailable')),
          events.listEventForAllNamespaces(),
        ];
        const responses = await Promise.allSettled([...requests, ...optionalRequests]);
        const values = responses.map(response => response.status === 'fulfilled' ? response.value.body?.items || [] : []);
        const [nodesResponse, eventsResponse] = responses.slice(requests.length);
        if (eventsResponse.status === 'rejected') capabilities.events = false;
        if (nodesResponse.status === 'rejected') capabilities.clusterNodes = false;
        if (responses.slice(0, requests.length).some(response => response.status === 'rejected')) {
          throw responses.find(response => response.status === 'rejected').reason;
        }
        const [deployments, statefulSets, daemonSets, services, pods, configMaps, secrets, persistentVolumeClaims, ingresses, clusterNodes, eventItems] = values;
        ...
        const nodes = [
          ...
          // Nodes are cluster-scoped, so they are never filtered by namespace.
          ...clusterNodes.map(item => nodeFromItem(contextName, item, 'node', clusterNodeHealth(item))),
        ].filter(Boolean);
        ...
        const rawById = new Map([...workloadItems, ...services, ...pods, ...configurationItems.map(([item]) => item), ...ingresses, ...clusterNodes].map(item => [item.metadata?.uid, item]));
        ...
        // Services take precedence over a same-named workload as the resolvable target of an internal
        // DNS/env reference, since that is what Kubernetes itself actually routes traffic to.
        const namedNodesByNamespace = new Map();
        for (const node of [...workloads, ...serviceNodes]) namedNodesByNamespace.set(`${node.namespace}:${node.name.toLowerCase()}`, node);
        for (const workload of workloads) {
          ...
          for (const reference of workloadServiceEnvReferences(rawById.get(workload.nativeId) || {})) {
            const namespace = reference.namespace || workload.namespace;
            const target = namedNodesByNamespace.get(`${namespace}:${reference.service.toLowerCase()}`);
            if (target && target.id !== workload.id) {
              result.relationships.push(relationship(contextName, workload, target, 'calls', { type: reference.evidence, key: reference.key }));
            }
          }
        }
        ...
        const clusterNodesByName = new Map(nodes.filter(item => item.resourceType === 'node').map(item => [item.name, item]));
        for (const pod of podNodes) {
          const target = clusterNodesByName.get(rawById.get(pod.nativeId)?.spec?.nodeName);
          if (target) result.relationships.push(relationship(contextName, pod, target, 'runs_on', { type: 'pod_node_assignment' }));
        }
        ...
        const warnings = eventItems.filter(event => event.type === 'Warning').map(event => ({
          ...
          regardingNamespace: event.regarding?.namespace || event.metadata?.namespace || '',
        }));
...
module.exports = {
  KubernetesAdapter, labelsMatch, nodeFromItem, stableId, workloadReferences, workloadServiceEnvReferences,
};