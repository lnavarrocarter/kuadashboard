...
function canonicalFromApm(application, resource) {
  ...
  return canonicalResource({
    ...
    scopeId: globallyScoped ? '' : (awsScope.scopeId || resource.kubeContext || ''),
    location: globallyScoped ? '' : (awsScope.location || (provider === 'kubernetes' ? '' : application.region || '')),
...
class ApplicationRegistryService {
  ...
  _reconcile(application) {
    ...
    const projectDocuments = projectIds
      ...
      .filter(project => project?.profileId === application.profileId)
    ...
    for (const item of projectDocuments) {
      ...
      item.document.nodes.map(node => apmProjectionFromNode(application, node)).filter(Boolean)
        .forEach(resource => {
    ...
    for (const item of projectDocuments) {
      for (const node of item.document.nodes) {
        ...
        architectureNodeReferences.push(nodeReference);
        nodeResourceIds.set(nodeReference, registered.id);
      }
    }
    this.database.pruneRegistryMemberships(application.id, 'architecture_node', architectureNodeReferences);
