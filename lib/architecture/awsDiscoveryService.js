...
class ArchitectureAwsDiscoveryService {
  ...
  async importSelection(projectId, input) {
    ...
    const selectedNodes = preview.nodes.filter(node => selected.has(node.id));
    ...
    // Recomputed here rather than trusting the preview, which may have been cached before the
    // project had these resources.
    this.markExistingNodes(preview, projectId);
    ...
    // reconcileDiscoveryImport merges them onto the existing nodes by identity.
