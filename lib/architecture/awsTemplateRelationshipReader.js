...
function templateRelationships(body, stackName = '', externalReferences = null) {
  ...
  // Fn::ImportValue points at another stack's Output export, which this reader cannot resolve on its own
  // (would need ListExports/DescribeStacks for every other stack). Surface it as a recommendation instead
  // of silently dropping the dependency, so the caller can suggest "this stack also needs stack X".
  function noteExternalReference(sourceLogicalId, exportName, path) {
    if (!exportName || !externalReferences) return;
    const key = `${sourceLogicalId}:${exportName}`;
    if (seenExternalReferences.has(key)) return;
    seenExternalReferences.add(key);
    externalReferences.push({ stackName, sourceLogicalId, exportName, path });
  }

  function importValueName(value) {
    const importValue = value?.['Fn::ImportValue'];
    if (typeof importValue === 'string') return importValue;
    const substitution = importValue?.['Fn::Sub'];
    return typeof substitution === 'string' ? substitution : null;
  }

  function restApiPath(resourceId, visited = new Set()) {
    if (!resourceId || visited.has(resourceId)) return '/';
    visited.add(resourceId);
    const resource = resources[resourceId];
    if (resource?.Type !== 'AWS::ApiGateway::Resource') return '/';
    const pathPart = typeof resource.Properties?.PathPart === 'string' ? resource.Properties.PathPart : '';
    const parentId = resource.Properties?.ParentId?.Ref;
    const parentPath = parentId ? restApiPath(parentId, visited) : '/';
    return `${parentPath === '/' ? '' : parentPath}/${pathPart}` || '/';
  }

  function apiRouteDetails(sourceLogicalId) {
    const source = resources[sourceLogicalId];
    if (source?.Type === 'AWS::ApiGateway::Method') {
      const method = String(source.Properties?.HttpMethod || 'ANY').toUpperCase();
      const path = restApiPath(source.Properties?.ResourceId?.Ref);
      return { method, routePath: path, route: `${method} ${path}` };
    }
    if (source?.Type === 'AWS::ApiGatewayV2::Route') {
      const route = String(source.Properties?.RouteKey || '$default');
      const separator = route.indexOf(' ');
      return separator > 0
        ? { method: route.slice(0, separator).toUpperCase(), routePath: route.slice(separator + 1), route }
        : { method: 'ANY', routePath: route, route };
    }
    return null;
  }
  ...
  function relationType(sourceLogicalId, targetLogicalId, path) {
    ...
    if (sourceType === 'AWS::ApiGateway::Method' && path.includes('.Properties.Integration.Uri')) return 'routes_to';
    if (sourceType === 'AWS::ApiGatewayV2::Route' && path.includes('.Properties.Target')) return 'routes_to';
  ...
  function add(sourceLogicalId, targetLogicalId, path, intrinsic, type = relationType(sourceLogicalId, targetLogicalId, path), confidence = 0.95, details = null) {
    ...
    current.evidence.push({ type: 'cloudformation_reference', path, intrinsic, ...(details || apiRouteDetails(sourceLogicalId) || {}) });
  ...
  function addHttpApiRoute(logicalId, properties) {
    const details = apiRouteDetails(logicalId);
    for (const integrationId of references(properties.Target)) {
      const integration = resources[integrationId];
      if (integration?.Type !== 'AWS::ApiGatewayV2::Integration') continue;
      for (const fn of references(integration.Properties?.IntegrationUri)) {
        if (resources[fn]?.Type === 'AWS::Lambda::Function') {
          add(logicalId, fn, `Resources.${logicalId}.Properties.Target`, 'AWS::ApiGatewayV2::Integration', 'routes_to', 0.99, details);
        }
      }
    }
  }
  ...
  function visit(sourceLogicalId, value, path) {
    ...
    noteExternalReference(sourceLogicalId, importValueName(value), path);
  ...
  for (const [logicalId, resource] of Object.entries(resources)) {
    ...
    if (resource.Type === 'AWS::ApiGatewayV2::Route') addHttpApiRoute(logicalId, resource.Properties || {});
    ...
    async analyze({ profileId, region, stackNames }) {
      ...
      for (const stackName of [...new Set(stackNames || [])]) {
        try {
          ...
          requests += 1;
          relationships.push(...templateRelationships(response.TemplateBody, stackName, externalReferences));
        } catch (error) {
      ...
      return { requests, relationships, externalReferences, failures };
    },
  };
}
