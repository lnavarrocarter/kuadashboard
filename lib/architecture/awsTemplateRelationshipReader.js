...
function templateRelationships(body, stackName = '') {
  ...
  function relationType(sourceLogicalId, path) {
    const resourceType = resources[sourceLogicalId]?.Type;
    if (resourceType === 'AWS::Events::Rule' && path.includes('.Properties.Targets')) return 'triggers';
    if (resourceType === 'AWS::StepFunctions::StateMachine' && path.includes('.Properties.Definition')) return 'invokes';
    if (resourceType === 'AWS::ECS::Service' && path.includes('.Properties.Cluster')) return 'runs_on';
    return 'depends_on';
  }

  function add(sourceLogicalId, targetLogicalId, path, intrinsic, type = relationType(sourceLogicalId, path), confidence = 0.95) {
