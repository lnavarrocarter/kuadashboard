...
const LAYOUT_MODES = new Set(['request-flow', 'resource-type', 'provider-lanes', 'provider-resource']);
...
function normalizeNodes(value) {
  return normalizeCollection(value, 'node').map(node => {
    if (String(node.provider || '').trim().toLowerCase() !== 'kubernetes' && node.resourceType !== 'kubernetes') {
      return node;
    }
    ...
    return resourceType ? { ...node, resourceType } : node;
  });
}
...
function normalizeView(value = {}) {
  ...
  return {
    ...
    showHealthOverlay: value.showHealthOverlay === true,
    showMetricsOverlay: value.showMetricsOverlay === true,
    showCollectionOverlay: value.showCollectionOverlay === true,
    showTraceOverlay: value.showTraceOverlay === true,
    showEventsOverlay: value.showEventsOverlay === true,
    providerFilter: optionalString(value.providerFilter) || 'all',
    kubeContextFilter: optionalString(value.kubeContextFilter),
    namespaceFilter: optionalString(value.namespaceFilter),
    relationTypeFilter: optionalString(value.relationTypeFilter) || 'all',
    relationStatusFilter: optionalString(value.relationStatusFilter) || 'all',
  };
}
