...
test('normalizes AWS deployment resources as deterministic evidence-backed candidates', async () => {
  ...
  try {
    ...
    assert.deepEqual(first.relationshipAnalysis.supportedEvidence, [
      'cloudformation_reference', 'aws_inventory', 'eventbridge_target', 'lambda_event_source_mapping', 'asl_reference', 'lambda_environment_variable', 'iam_role_policy', 'lambda_code_reference', 'sns_subscription',
    ]);
  } finally {
...
test('surfaces a cross-stack Fn::ImportValue dependency as a recommendation to add the exporting stack', async () => {
  const database = new ArchitectureDatabase({ filePath: ':memory:' });
  const graphService = new ArchitectureGraphService({ database });
  const deploymentReader = {
  ...
  const relationshipReader = {
    async analyze() {
      return {
        ...
        failures: [],
        relationships: [],
        externalReferences: [{ stackName: 'orders', sourceLogicalId: 'Worker', exportName: 'network-QueueArn', path: 'Resources.Worker.Properties.Environment.Variables.QUEUE_ARN' }],
      };
    },
  };
  const inventoryReader = { async analyze() { return { accountId: '', resources: [], relationships: [], failures: [], requests: 0, truncated: false }; } };
  const service = new ArchitectureAwsDiscoveryService({ deploymentReader, inventoryReader, relationshipReader, graphService });
  try {
    const preview = await service.preview({ profileId: 'local:dev', region: 'us-east-1', accountId: '123456789012', stackNames: ['orders'] });
    assert.equal(preview.relationshipAnalysis.crossStackReferences.length, 1);
    assert.equal(preview.relationshipAnalysis.crossStackReferences[0].exportName, 'network-QueueArn');
    assert.equal(preview.relationshipAnalysis.crossStackReferences[0].sourceNodeId, preview.nodes[0].id);
  } finally {
    database.close();
  }
});

test('keeps relationships that connect a newly selected resource to one already in the project', async () => {
  const subject = fixture();
  try {
    const input = { profileId: 'local:dev', region: 'us-east-1', accountId: '123456789012', stackNames: ['orders'] };
    const initialPreview = await subject.service.preview(input);
    const workerNode = initialPreview.nodes.find(node => node.name === 'orders-worker');
    const queueNode = initialPreview.nodes.find(node => node.name === 'orders' && node.resourceType === 'sqs');

    const project = subject.database.createProject({ profileId: 'local:dev', name: 'orders-architecture' });
    const initialGraph = subject.database.getGraph(project.id);
    subject.database.saveGraph(project.id, { ...initialGraph.document, nodes: [workerNode] }, { expectedRevision: initialGraph.revision });
    const stored = subject.database.getGraph(project.id);

    // Only the queue is selected: the worker is already in the project, so the panel does not offer
    // it. The relationship between them must still be imported.
    const graph = await subject.service.importSelection(project.id, {
      ...input, projectId: project.id, selectedNodeIds: [queueNode.id], expectedRevision: stored.revision,
    });

    const edge = graph.document.edges.find(item =>
      (item.sourceNodeId === workerNode.id && item.targetNodeId === queueNode.id) ||
      (item.sourceNodeId === queueNode.id && item.targetNodeId === workerNode.id));
    assert.ok(edge, 'the relationship with the already-present resource must survive the import');
    // The already-present resource must not be duplicated by travelling as context.
    assert.equal(graph.document.nodes.filter(node => node.id === workerNode.id).length, 1);
  } finally {
    subject.database.close();
  }
});

test('marks preview resources already present in the project graph, so the panel can show them as already added', async () => {
  const subject = fixture();
  try {
    const input = { profileId: 'local:dev', region: 'us-east-1', accountId: '123456789012', stackNames: ['orders'] };
    const initialPreview = await subject.service.preview(input);
    ...
    const project = subject.database.createProject({ profileId: 'local:dev', name: 'orders-architecture' });
    const graph = subject.database.getGraph(project.id);
    subject.database.saveGraph(project.id, { ...graph.document, nodes: [workerNode] }, { expectedRevision: graph.revision });

    const preview = await subject.service.preview({ ...input, projectId: project.id });

    const previewWorkerNode = preview.nodes.find(node => node.id === workerNode.id);
    const previewQueueNode = preview.nodes.find(node => node.id === queueNode.id);
    assert.equal(previewWorkerNode.alreadyInGraph, true);
    assert.equal(previewWorkerNode.existingNodeId, workerNode.id);
    assert.equal(previewQueueNode.alreadyInGraph, false);
    assert.equal(previewQueueNode.existingNodeId, null);
  } finally {
    subject.database.close();
  }
});

test('never marks resources as already added when no project is given (regional inventory without a linked project)', async () => {
  const subject = fixture();
  try {
    const preview = await subject.service.preview({ profileId: 'local:dev', region: 'us-east-1', accountId: '123456789012', stackNames: ['orders'] });
    assert.ok(preview.nodes.every(node => node.alreadyInGraph === false && node.existingNodeId === null));
  } finally {
    subject.database.close();
  }
});
