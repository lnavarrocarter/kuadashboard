...
test('normalizes AWS deployment resources as deterministic evidence-backed candidates', async () => {
  ...
  try {
    ...
    assert.deepEqual(first.relationshipAnalysis.supportedEvidence, [
      'cloudformation_reference', 'aws_inventory', 'eventbridge_target', 'lambda_event_source_mapping', 'asl_reference', 'lambda_environment_variable', 'iam_role_policy', 'lambda_code_reference',
    ]);
  } finally {
