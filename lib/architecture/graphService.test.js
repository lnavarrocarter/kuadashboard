...
test('persists normalized canvas view preferences', () => {
  ...
  const next = applyGraphOperation(original, {
    type: 'view.set',
    value: { layoutMode: 'provider-resource', layoutDirection: 'vertical', showEdgeLabels: true, relationTypeFilter: 'routes_to', relationStatusFilter: 'suggested' },
  });

  assert.deepEqual(next.view, {
    layoutMode: 'provider-resource', layoutDirection: 'vertical', showEdgeLabels: true,
    showHealthOverlay: false, showMetricsOverlay: false, showCollectionOverlay: false, showTraceOverlay: false,
    providerFilter: 'all', kubeContextFilter: '', namespaceFilter: '', relationTypeFilter: 'routes_to', relationStatusFilter: 'suggested',
  });
