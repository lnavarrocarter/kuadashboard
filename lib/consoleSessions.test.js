...
const ssh = { provider: 'aws', transport: 'ssh', profileId: 'profile-1', environment: 'dev', applicationId: 'app-1', target: { host: 'host', user: 'ec2-user' } };
...
test('SSM authority resolves through the generic AWS credential path, not the raw SSH/RDP secret store', async () => {
  const ssm = { provider: 'aws', transport: 'ssm', profileId: 'profile-1', target: { instanceId: 'i-123' } };
  const resolveAwsConfig = async profileId => {
    assert.equal(profileId, 'profile-1');
    return { credentials: { accessKeyId: 'AKIA-fake', secretAccessKey: 'secret-fake' }, region: 'us-east-1' };
  };
  const resolved = await resolveSession(ssm, { resolveAwsConfig });
  assert.equal(resolved.authority.ssm.region, 'us-east-1');
  assert.equal(resolved.authority.ssm.credentials.accessKeyId, 'AKIA-fake');
  assert.equal(resolved.authority.ssh, undefined);
  assert.equal(resolved.authority.rdp, undefined);

  await assert.rejects(
    resolveSession(ssm, { resolveAwsConfig: async () => { throw new Error('Credential profile not found'); } }),
    /Credential profile not found/,
  );
});

test('GCP authority resolves via the generic GCP credential path (routes/gcp.js\'s resolveGcpAuth)', async () => {
  const gcpLogs = { provider: 'gcp', transport: 'logs', profileId: 'profile-1', project: 'proj-1', region: 'us-central1', target: { name: 'my-svc' } };
  const resolveGcpAuth = async profileId => {
    assert.equal(profileId, 'profile-1');
    return { projectId: 'proj-1', credentials: { client_email: 'sa@proj.iam.gserviceaccount.com' } };
  };
  const resolved = await resolveSession(gcpLogs, { resolveGcpAuth });
  assert.equal(resolved.authority.gcp.projectId, 'proj-1');
  assert.equal(resolved.authority.ssh, undefined);

  await assert.rejects(
    resolveSession(gcpLogs, { resolveGcpAuth: async () => { throw new Error('GCP_PROJECT_ID is required'); } }),
    /GCP_PROJECT_ID is required/,
  );
});

test('gcp-logs validates without a client-supplied project — it is resolved from the profile server-side', async () => {
  const { validateSession } = await contract;
  const { session } = validateSession({ provider: 'gcp', transport: 'logs', profileId: 'profile-1', region: 'us-central1', target: { name: 'my-svc' } });
  assert.equal(session.project, null);
  const resolveGcpAuth = async () => ({ projectId: 'resolved-from-profile', credentials: {} });
  const resolved = await resolveSession({ provider: 'gcp', transport: 'logs', profileId: 'profile-1', region: 'us-central1', target: { name: 'my-svc' } }, { resolveGcpAuth });
  assert.equal(resolved.authority.gcp.projectId, 'resolved-from-profile');
});

test('Vercel authority resolves via the generic Vercel credential path (routes/vercel.js\'s resolveVercelAuth)', async () => {
  const vercelLogs = { provider: 'vercel', transport: 'deployment-logs', profileId: 'profile-1', target: { name: 'dpl_123' } };
  const resolveVercelAuth = async profileId => {
    assert.equal(profileId, 'profile-1');
    return { token: 'tok-1', teamId: 'team-1' };
  };
  const resolved = await resolveSession(vercelLogs, { resolveVercelAuth });
  assert.equal(resolved.authority.vercel.token, 'tok-1');
  assert.equal(resolved.authority.vercel.teamId, 'team-1');

  await assert.rejects(
    resolveSession(vercelLogs, { resolveVercelAuth: async () => { throw new Error('Credential profile not found'); } }),
    /Credential profile not found/,
  );
});
...
test('preflight strips secrets; tickets are one-use, path-bound and expire', async () => {
  ...
  assert.equal(entries[0].details.applicationId, 'app-1');
