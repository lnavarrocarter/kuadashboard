...
const PROVIDERS = new Set(['generic', 'aws', 'gcp', 'vercel']);
...
class ApmDatabase {
  ...
  listRegistryResources(applicationId) {
    return this.db.prepare(`
      SELECT DISTINCT r.* FROM kua_registry_resources r
      JOIN kua_registry_memberships m ON m.resource_id = r.id
      WHERE m.application_id = ? ORDER BY r.provider, r.resource_type, r.display_name
