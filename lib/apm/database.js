...
const RESOURCE_TYPES = new Set(['lambda', 'kubernetes', 'sqs', 'eventbridge', 'stepfunctions', 'ecs']);
...
class ApmDatabase {
  ...
  createApplication(input) {
    ...
    this.db.prepare(`
      INSERT INTO apm_applications (
        id, profile_id, region, name, environment, team, polling_enabled,
        poll_interval_minutes, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, 30, ?, ?)
    `).run(
      application.id, application.profileId, application.region, application.name,
  ...
  listApplications({ profileId, region } = {}) {
    const conditions = [];
    const values = [];
  ...
  addResource(applicationId, input) {
    ...
    if (!['manual', 'tags', 'labels', 'deployment'].includes(resource.associationSource)) throw new Error('Unsupported association source');
    this.db.prepare(`
      INSERT INTO apm_resources (
        id, application_id, resource_type, resource_key, arn, kube_context, namespace,
        kind, name, service, log_group, metadata_json, association_source, enabled,
    ...
    `).run(
      resource.id, applicationId, resource.type, resource.key, input.arn || null,
