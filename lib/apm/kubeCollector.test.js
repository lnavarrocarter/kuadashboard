...
function fixture({ metricsError = null, prometheus = false } = {}) {
  ...
  const clients = {
    ...
    core: {
      ...
      async connectGetNamespacedServiceProxyWithPath(name, namespace, target, path) {
        assert.equal(name, 'prometheus');
        assert.equal(namespace, 'monitoring');
        assert.equal(target, 'prometheus');
        if (decodeURIComponent(path).includes('container_cpu_usage_seconds_total')) {
          return { body: { data: { result: [{ value: [0, '0.42'] }] } } };
        }
        return { body: { data: { result: [{ value: [0, '123456'] }] } } };
      },
    },
    custom: {
