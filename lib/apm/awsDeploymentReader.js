...
function createAwsDeploymentReader({
  ...
  async function send(client, command, profileId, region) {
  ...
  return {
    async listDeployments({ profileId, region }) {
      ...
      let requests = 0;
      do {
        const response = await send(cloudFormation, new ListStacksCommand({
    ...
    async preview({ profileId, region, stackNames }) {
      ...
      for (const stackName of [...new Set(stackNames)]) {
        ...
        const summaries = [];
        do {
          const response = await send(cloudFormation, new ListStackResourcesCommand({ StackName: stackName, NextToken: nextToken }), profileId, region);
          requests += 1;
          summaries.push(...(response.StackResourceSummaries || []));
          nextToken = response.NextToken;
        } while (nextToken);
        resources.push(...deploymentResources(stackName, summaries, { includeAllResources }));
        ...
        for (const cluster of clusters) {
          ...
          const serviceArns = [];
          do {
            const response = await send(ecs, new ListServicesCommand({
          ...
          for (let index = 0; index < serviceArns.length; index += 10) {
            const response = await send(ecs, new DescribeServicesCommand({
              cluster: cluster.PhysicalResourceId,
      ...
      return {
        accountId: uniqueResources.map(resource => String(resource.arn || '').split(':')[4] || '')
