...
test('lists workloads only for explicitly selected compatible contexts', async () => {
  const contexts = [
    { name: 'first', cluster: 'first-cluster' },
    { name: 'second', cluster: 'second-cluster' },
  ];
  const calls = [];
  const configBuilder = contextName => ({
    kubeConfig: {
      getContexts: () => contexts,
      getClusters: () => [
        { name: 'first-cluster', server: 'https://first.eks.amazonaws.com' },
        { name: 'second-cluster', server: 'https://second.eks.amazonaws.com' },
      ],
      makeApiClient: () => ({
        listDeploymentForAllNamespaces: async () => { calls.push(contextName); return { body: { items: [] } }; },
        listStatefulSetForAllNamespaces: async () => ({ body: { items: [] } }),
  ...
  const reader = createEksWorkloadReader({ configBuilder, AppsV1Api: class {} });
  assert.deepEqual(reader.listContexts(), [
    { id: 'first', name: 'first', cluster: 'first-cluster', server: 'https://first.eks.amazonaws.com' },
    { id: 'second', name: 'second', cluster: 'second-cluster', server: 'https://second.eks.amazonaws.com' },
  ]);
  await reader.listWorkloads({ contexts: ['second'] });
