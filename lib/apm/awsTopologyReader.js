...
function resourceMatchesReference(resource, reference) {
  if (!resource || !reference || resource.provider && resource.provider !== 'aws') return false;
  if (resource.type !== reference.type) return false;
  const referenceArn = String(reference.arn || '').toLowerCase();
  const referenceName = String(reference.name || '').toLowerCase();
  if (!referenceArn && !referenceName) return false;
  const identities = [resource.arn, resource.key, resource.name].filter(Boolean).map(value => String(value).toLowerCase());
  return identities.some(identity => (referenceArn && identity === referenceArn) ||
    (referenceName && lastArnSegment(identity).toLowerCase() === referenceName));
}
...
function createAwsTopologyReader({
  ...
  return {
    async analyze({ application, resources, edges = [] }) {
      ...
      let requests = 0;
      for (const source of resources.filter(resource => (!resource.provider || resource.provider === 'aws') &&
        resource.type === 'stepfunctions' && resource.enabled && resource.arn)) {
        let response;
        try {
