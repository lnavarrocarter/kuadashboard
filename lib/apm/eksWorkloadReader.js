  ...
  return {
    listContexts({ provider = 'aws' } = {}) {
      ...
      return kubeConfig.getContexts()
        ...
        .map(context => ({ id: context.name, name: context.name, cluster: context.cluster, server: clusters.get(context.cluster)?.server || '' }))
        .sort((left, right) => left.name.localeCompare(right.name));
    },

    async listWorkloads({ provider = 'aws', contexts: requestedContexts } = {}) {
      const availableContexts = this.listContexts({ provider });
