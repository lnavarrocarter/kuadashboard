...
test('creates a private WAL database and migrates once', () => {
  ...
  try {
    assert.equal(subject.database.health().schemaVersion, 14);
...
test('supports many-to-many application and architecture associations', () => {
  const subject = fixture();
  try {
    const first = subject.database.createApplication({
      profileId: 'local:dev', region: 'us-east-1', name: 'orders', provider: 'aws',
    });
    const second = subject.database.createApplication({
      profileId: 'local:dev', region: 'us-east-1', name: 'billing', provider: 'aws',
    });

    subject.database.updateArchitectureProjectLink(first.id, 'architecture-orders');
    subject.database.updateArchitectureProjectLink(first.id, 'architecture-shared');
    subject.database.updateArchitectureProjectLink(second.id, 'architecture-shared');

    assert.deepEqual(subject.database.getApplication(first.id).architectureProjectIds, [
      'architecture-orders', 'architecture-shared',
    ]);
    assert.deepEqual(
      subject.database.listApplicationsByArchitectureProjectId('architecture-shared').map(item => item.name),
      ['billing', 'orders'],
    );

    subject.database.unlinkArchitectureProject(first.id, 'architecture-shared');
    assert.deepEqual(subject.database.getApplication(first.id).architectureProjectIds, ['architecture-orders']);
    assert.deepEqual(subject.database.listApplicationsByArchitectureProjectId('architecture-shared').map(item => item.name), ['billing']);
  } finally {
    subject.close();
  }
});
...
test('stores the AWS resource types Architecture can discover, so they reach Observability', () => {
  const subject = fixture();
  try {
    const application = subject.database.createApplication({
      profileId: 'local:dev', region: 'us-east-1', name: 'orders', environment: 'dev', pollingEnabled: false,
    });
    // EC2 and S3 report CloudWatch metrics; EKS and the rest are inventory that used to be
    // rejected outright by the resource_type constraint and never appeared in Observability.
    for (const type of ['ec2', 'eks', 'rds', 'apigateway', 'cloudfront', 'autoscaling', 'elasticache']) {
      const resource = subject.database.addResource(application.id, {
        type, key: `arn:aws:${type}:us-east-1:123:resource/${type}`, name: type, associationSource: 'architecture',
      });
      assert.equal(resource.type, type);
    }
    assert.equal(subject.database.listResources(application.id).length, 7);
    assert.throws(
      () => subject.database.addResource(application.id, { type: 'not-a-service', key: 'k', name: 'n', associationSource: 'manual' }),
