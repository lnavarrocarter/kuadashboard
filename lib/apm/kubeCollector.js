...
const COLLECTED_KINDS = new Set(['deployment', 'statefulset', 'daemonset', 'service', 'pod', 'node', 'ingress']);
...
function podMatcher(podNames) {
  const names = [...podNames];
  return names.length === 1 ? `pod="${labelValue(names[0])}"` : `pod=~"${names.map(regexEscape).join('|')}"`;
}
...
class KubeCollector {
  ...
  async _collectPrometheusUsage(kubeConfig, namespace, podNames, clients) {
    const scope = `namespace="${namespace}",${podMatcher(podNames)},container!="",container!="POD"`;
  ...
  // How much log the workload is producing on disk, which the Kubernetes Metrics API never reports.
  // Best-effort: a cluster without Prometheus simply gets no log metric, and is only probed once.
  async _collectLogVolume(kubeConfig, namespace, podNames, clients) {
    if (this._prometheusUnavailable) return null;
    try {
      const { values } = await this._runPrometheusQueries(kubeConfig, clients, [
        `sum(kubelet_container_log_filesystem_used_bytes{namespace="${namespace}",${podMatcher(podNames)}})`,
      ]);
      return values[0];
    } catch (_) {
      this._prometheusUnavailable = true;
      return null;
    }
  }
  ...
  async collect({ resource }) {
    ...
    const kind = String(resource.kind || '').toLowerCase();
    // A Secret or ConfigMap has nothing to measure: reporting it as topology-only keeps it in the
    // graph without raising a collection error for every one of them on every run.
    if (!COLLECTED_KINDS.has(kind)) return { status: 'topology_only', requests: 0, backlog: false };
    ...
    const logBytes = await this._collectLogVolume(kubeConfig, namespace, podNames, clients);
    if (logBytes != null) buckets.push(metric('log_bytes', 'bytes', logBytes));
