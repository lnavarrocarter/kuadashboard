...
class KubeCollector {
  ...
  async _collectPrometheusUsage(namespace, podNames, clients) {
    if (!clients.core?.listServiceForAllNamespaces || !clients.core?.connectGetNamespacedServiceProxyWithPath) {
    ...
    for (const service of services) {
      for (const target of prometheusServiceCandidates(service)) {
        try {
          const [cpu, memory] = await Promise.all(queries.map(query => clients.core.connectGetNamespacedServiceProxyWithPath(
            service.metadata.name, service.metadata.namespace, target, prometheusQueryPath(query),
          )));
          return {
  ...
  async collect({ resource }) {
    ...
    try {
      ...
      try {
        const usage = await this._collectPrometheusUsage(namespace, podNames, clients);
