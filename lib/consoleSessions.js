...
async function resolveSession(input, { store, kubeConfig, resolveAwsConfig, resolveGcpAuth, resolveVercelAuth } = {}) {
  ...
  } else if (session.provider === 'aws' && session.transport === 'ssm') {
    // SSM is a real AWS API call (StartSession/TerminateSession), not a raw socket like
    // SSH/RDP — resolve through the same generic AWS credential path every other AWS
    // route already uses, not the credential store's raw SSH/RDP secret branch below.
    resolveAwsConfig ||= require('./awsProfileResolver').resolveAwsConfig;
    authority.ssm = await resolveAwsConfig(session.profileId);
  ...
  } else if (session.provider === 'gcp') {
    // Cloud Logging is a real GCP API call — resolve through the same generic GCP
    // credential path every routes/gcp.js endpoint already uses, mirroring the SSM branch.
    resolveGcpAuth ||= require('../routes/gcp').resolveGcpAuth;
    authority.gcp = await resolveGcpAuth(session.profileId);
  } else if (session.provider === 'vercel') {
    resolveVercelAuth ||= require('../routes/vercel').resolveVercelAuth;
    authority.vercel = await resolveVercelAuth(session.profileId);
  }
...
function auditSession(audit, action, session) {
  ...
  audit.log({
    ...
    details: { provider: session.provider, environment: session.environment, applicationId: session.applicationId, transport: session.transport },
