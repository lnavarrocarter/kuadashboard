 ...
 *
 *   KeyringStore — OS native keychain via `@napi-rs/keyring` (pre-built N-API bindings,
 *                   no recompilation needed). Uses macOS Keychain / Windows DPAPI / libsecret.
 *
 * The active backend is selected at startup:
 *   - If `process.env.KUADASHBOARD_STORE === 'keytar'` → KeyringStore
 *   - If running as Electron (process.versions.electron exists) → KeyringStore
 ...
 *   provider:  'gcp' | 'aws' | 'vercel' | 'generic',
...
// ─── KeyringStore ─────────────────────────────────────────────────────────────
// Uses @napi-rs/keyring — pre-built N-API bindings to the OS native keychain:
//   macOS  → Keychain Services (AES-256 encrypted by the OS)
//   Windows → Credential Manager (DPAPI)
//   Linux  → libsecret / KWallet
// No recompilation needed for different Electron/Node versions.
...
class KeyringStore {
  constructor() {
    try {
      ...
      throw new Error(
        'KeyringStore requires the `@napi-rs/keyring` package. ' +
        'Install it with: npm install @napi-rs/keyring'
      );
    }
  }

  /** Returns a keyring Entry for a given profile+key combination */
  _entry(profileId, keyName) {
    return new this._Entry(KEYTAR_SERVICE, `${profileId}::${keyName}`);
  }
  ...
  async getProfile(id) {
    ...
    for (const kn of (profile.keyNames || [])) {
      const val = this._entry(id, kn).getPassword();
  ...
  async getRawKeys(id) {
    ...
    const keys = {};
    for (const kn of (profile.keyNames || [])) {
      keys[kn] = this._entry(id, kn).getPassword() || '';
    }
    return keys;
  }
  ...
  async createProfile({ name, provider, category, keys = {}, meta = {} }) {
    ...
    for (const [kn, val] of Object.entries(keys)) {
      this._entry(id, kn).setPassword(val);
    }
    const profile = {
      id, name,
      provider:  provider || 'generic',
  ...
  async deleteProfile(id) {
    ...
    for (const kn of (profile.keyNames || [])) {
      try { this._entry(id, kn).deletePassword(); } catch (_) {}
    }
...
function createStore() {
  ...
  if (forceKeytar || isElectron) {
    try {
      ...
      console.log('[credentialStore] Using KeyringStore (OS native keychain)');
      return store;
    } catch (err) {
      console.warn('[credentialStore] KeyringStore unavailable, falling back to FileStore:', err.message);
    }
  }
...
module.exports = { getStore, FileStore, KeyringStore };
